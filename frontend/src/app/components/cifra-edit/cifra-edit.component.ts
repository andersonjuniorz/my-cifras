...
@Component({
  ...
  template: `
    <div class="max-w-3xl mx-auto px-4 py-8">
      <div class="mb-6">
        <a [routerLink]="['/cifras', id]" class="text-sm text-zinc-500 hover:text-zinc-300 transition-colors">&larr; Voltar</a>
      </div>

      @if (carregando()) {
        <div class="text-center py-16 text-zinc-600">Carregando...</div>
      } @else {
        <h1 class="text-2xl font-bold tracking-tight mb-6">Editar Cifra</h1>
        ...
        <form (ngSubmit)="salvar()" class="space-y-5">
          ...
          <div>
            ...
            <textarea [(ngModel)]="form.conteudo" name="conteudo" required rows="16"
                      class="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-4 py-3 text-sm text-zinc-100 placeholder-zinc-600 focus:outline-none focus:border-amber-500/50 transition-colors font-mono leading-relaxed resize-y"></textarea>
          </div>
          ...
          <div>
            ...
            <textarea [(ngModel)]="form.observacoes" name="observacoes" rows="3"
                      class="w-full bg-zinc-900 border border-zinc-800 rounded-lg px-4 py-3 text-sm text-zinc-100 placeholder-zinc-600 focus:outline-none focus:border-amber-500/50 transition-colors resize-y"></textarea>
          </div>
          ...
          <div class="flex items-center gap-3 pt-2">
            ...
            <a [routerLink]="['/cifras', id]" class="text-sm text-zinc-500 hover:text-zinc-300 transition-colors">Cancelar</a>
          </div>
...
export class CifraEditComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.id) {
      this.cifraService.buscarPorId(this.id).subscribe({
        next: (data) => {
          this.form = data;
          if (data.acordesIds) {
            const ids = data.acordesIds.split(',').filter(Boolean).map(Number);
            this.acordeIds.set(new Set(ids));
          }
  ...
  salvar() {
    ...
    this.cifraService.atualizar(this.id, this.form).subscribe({
      next: () => {
        this.salvando.set(false);
        this.router.navigate(['/cifras', this.id]);
      },
      error: (err) => {
        this.salvando.set(false);
        this.erro.set('Erro ao salvar alteracoes. Tente novamente.');
        console.error(err);
      },
    });
  }
